...
bool DkBasicLoader::loadGeneral(const QString& filePath, QSharedPointer<QByteArray> ba, bool loadMetaData, bool fast) {
	...
	if (!imgLoaded && newSuffix.contains(QRegExp("(dcm|DCM)", Qt::CaseInsensitive))) {

		// load dcm here...
		imgLoaded = loadDICOMFile(mFile, ba);
		if (imgLoaded) mLoader = dcm_loader;
	}
...
/**
* Loads special DICOM files that are generated by the DCMTK.
* @param fileName the filename of the file to be loaded.
* @return bool true if the file could be loaded.
**/
bool DkBasicLoader::loadDICOMFile(const QString& filePath, QSharedPointer<QByteArray> ba){
	...
	try{
		...
		DcmFileFormat file;
		file.loadFile(filePath.toStdString().c_str());
		...
		if (dcimage != NULL)
		{
			...
			if (dcimage->getStatus() == EIS_Normal)
			{
				...
				if (pixelData != NULL)
				{
					...
					img = QImage(colored, dcimage->getWidth(), dcimage->getHeight(), QImage::Format_ARGB32).copy();
					delete[] colored;
				}
			}
			delete dcimage;
